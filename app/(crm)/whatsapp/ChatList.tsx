...
export function Avatar({ url, name, phone, size }: { url: string | null; name: string | null; phone: string; size: number }) {
  ...
  return (
    <div style={{
      width: size, height: size, borderRadius: '50%', background: '#374151', flexShrink: 0,
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      color: '#F9FAFB', fontSize: size * 0.4, fontWeight: 500,
  ...
  return (
    <>
      <div style={{
        ...
        <div style={{ flex: 1, color: '#F9FAFB', fontSize: 16, fontWeight: 500 }}>Conversas</div>
        {syncInfo && <span style={{ fontSize: 11, color: '#9CA3AF' }}>{syncInfo}</span>}
        <button
          ...
          style={{
            ...
            color: syncing ? '#9CA3AF' : '#9CA3AF', fontSize: 17,
      ...
      <div style={{ padding: '8px 12px', flexShrink: 0, background: '#0a0a0a' }}>
        <input
          value={search} onChange={e => onSearch(e.target.value)}
          placeholder="Pesquisar ou começar uma nova conversa"
          style={{
            width: '100%', background: '#111111', border: 'none', borderRadius: 8,
            padding: '8px 14px', fontSize: 13, color: '#F9FAFB', outline: 'none',
          }}
        />
      </div>
          ...
          return (
            <div key={c.id} onClick={() => onSelect(c.id)}
              ...
              style={{
                ...
                cursor: 'pointer', background: isActive ? '#1a1a1a' : 'transparent',
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                  <div style={{
                    flex: 1, minWidth: 0, color: '#F9FAFB', fontSize: 17,
                    overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                  }}>{c.name || c.phone}</div>
                  <div style={{ fontSize: 12, color: c.unread_count > 0 ? '#E53E3E' : '#9CA3AF', flexShrink: 0 }}>
                    {listTime(c.last_message_at)}
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 2 }}>
                  <div style={{
                    flex: 1, minWidth: 0, color: '#9CA3AF', fontSize: 14,
                    overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                  }}>
                  ...
                  {c.unread_count > 0 && (
                    <span style={{
                      background: '#E53E3E', color: '#0a0a0a', fontSize: 11, fontWeight: 600,
