...
- [ ] **Step 4: Conversation.tsx** — props `{ chat, messages, agentPaused, onToggleAgent, onSend, onLoadMore, userId }`.
  ...
  - Composer `#202c33`: botão anexo (📎 → `<input type=file hidden>`), textarea auto-height 1–5 linhas (`#2a3942`, sem borda, `#e9edef`), botão enviar (ícone ➤ verde). Enter envia, Shift+Enter quebra. Preview do arquivo selecionado com nome e botão ✕ antes de enviar. Enquanto envia: desabilita.
