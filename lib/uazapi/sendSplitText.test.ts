...
vi.mock('@/lib/redis/client', () => ({ getRedis: () => ({ set: async () => 'OK' }) }))

const fetchMock = vi.fn(async () => ({ ok: true, json: async () => ({ id: 'x', messageid: 'x' }) }))
...
describe('sendSplitText', () => {
  it('sem shouldStop envia todos os blocos (comportamento antigo)', async () => {
    ...
    expect(await p).toEqual({ sent: 3, total: 3 })
  ...
  it('para no bloco seguinte quando um humano assume no meio', async () => {
    ...
    paused = true
    await vi.runAllTimersAsync()
    expect(await p).toEqual({ sent: 1, total: 3 })
  ...
  it('pausado antes do primeiro bloco não envia nada', async () => {
    ...
    await vi.runAllTimersAsync()
    expect(await p).toEqual({ sent: 0, total: 3 })
